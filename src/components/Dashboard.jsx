...
export default function Dashboard() {
  ...
  const handleExecuteAnalysis = () => {
    ...
    const processData = (parsedMetrics = null) => {
      try {
        ...
        const isFXS1Signal = allNamesStr.includes("2603") || allNamesStr.includes("2607") || allNamesStr.includes("fxs1");
        
        let signalCode = "2379208";
        if (csvFile) {
          const match = csvFile.name.match(/\d+/);
          if (match) signalCode = match[0];
        }

        let targetSignalName = `Sinyal #${signalCode}`;
        let targetIndexName = `MT5 Signal - ${signalCode}`;

        if (isFXS1Signal) {
          targetSignalName = "FXS1";
          targetIndexName = "MT5 Signal - 001";
        }
        ...
        if (isFXS1Signal) {
          ...
          newOrUpdatedSignalData = {
            ...
            indexProvider: existingSignal ? existingSignal.indexProvider : `Provider #${signalCode}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="no-print flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex bg-slate-200 p-1 rounded-xl w-full md:w-fit space-x-1">
          <button onClick={() => handleTabChange('summary')} className={`flex-1 md:flex-none px-5 py-2 rounded-lg text-sm font-semibold transition-all flex items-center justify-center space-x-2 ${activeTab === 'summary' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
            <BarChart2 size={16} /> <span>Executive Summary & Laporan ({displayName})</span>
          </button>
        </div>
      ...
      {/* ADMIN AUTH & SETTINGS MODALS (PASSWORD DEFAULT DIHAPUS) */}
      {showAuthModal && (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 space-y-4 border animate-fadeIn">
            ...
            <form onSubmit={handlePasswordChange} className="space-y-3">
              <input type="password" value={newPasswordInput} onChange={(e) => setNewPasswordInput(e.target.value)} placeholder="Password Baru..." className="w-full p-2.5 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500" autoFocus />
              <div className="flex justify-end space-x-2 pt-2">
      ...
      {/* EXECUTIVE SUMMARY & COMPLETE REPORT VIEW */}
      {activeTab === 'summary' && (
        <div className="space-y-6 animate-fadeIn">
          
          {/* WIDGET INTERAKTIF: DYNAMIC BATCHING */}
          ...
          {/* 3. NARASI DETAIL LAPORAN INSTITUSIONAL CRO (DITAMPILKAN UTUH) */}
            ...
            <div className="space-y-5 text-sm text-slate-200 leading-relaxed">
              ...
              <div className="print-card bg-slate-800/60 p-4 rounded-xl border border-slate-700/80 space-y-2">
                ...
                <p className="text-xs text-slate-300">
                  Pertumbuhan akumulatif sinyal <strong>{displayName}</strong> sebesar <strong className="text-emerald-400">{data.growth}</strong> selama rekam jejak <strong className="text-emerald-400">{data.reliabilityWeeks} Minggu</strong> menunjukkan kurva ekuitas yang sangat teratur. Didukung oleh <strong>Calmar Ratio {data.calmarRatio}</strong> dan <strong>Recovery Factor {data.recoveryFactor}</strong>, kurva ekuitas membuktikan ekspansi modal terjadi secara organik tanpa lonjakan spekulatif berbahaya.
                </p>
              </div>
              ...
              <div className="print-card bg-slate-800/60 p-4 rounded-xl border border-slate-700/80 space-y-2">
                ...
                <p className="text-xs text-slate-300">
                  Berdasarkan audit mendalam struktur marjin, <strong>Deposit Load Maksimum tercatat pada {data.maxDepositLoad}%</strong>. Ini adalah validasi mutlak bahwa sistem **BEBAS DARI STRATEGI MARTINGALE MAUPUN GRID TOXIC**. Tidak ada penambahan volume lot eksponensial saat posisi mengalami kerugian.
                </p>
              </div>
              ...
              <div className="print-card bg-slate-800/60 p-4 rounded-xl border border-slate-700/80 space-y-2">
                ...
                <p className="text-xs text-slate-300">
                  Kapasitas optimal untuk alokasi dana copy trading pada sinyal ini diperkirakan mencapai <strong className="text-indigo-300">{data.fundCapacity}</strong> guna meminimalkan risiko pelebaran *spread* saat eksekusi lot institusional.
                </p>
              </div>
              ...
              <div className="print-card bg-indigo-900/50 p-4 rounded-xl border border-indigo-500/40 space-y-2">
                ...
                <p className="text-xs text-slate-200">
                  Sinyal <strong>{displayName}</strong> berhasil melewati seluruh standar uji kuantitatif komite investasi institusional dengan rekam jejak yang terverifikasi. Rekomendasi mutlak: <strong>APPROVED UNTUK ALOKASI DANA KELOLAAN</strong>.
                </p>
              </div>
